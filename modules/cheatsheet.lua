...
local function createView(html, title)
  ...
  -- Cap width at 1400px for readability on ultra-wide monitors
  local maxWidth = 1800
  local viewWidth = math.min(screenFrame.w * 0.94, maxWidth)
  local viewHeight = screenFrame.h * 0.65
  local viewRect = hs.geometry.rect(
    screenFrame.x + (screenFrame.w - viewWidth) / 2,
    screenFrame.y + (screenFrame.h - viewHeight) / 2,
    viewWidth,
    viewHeight
  )
...
local function showCheatsheet(name, visibleValue)
  ...
  -- Mark visible immediately so toggle can dismiss even while loading
  visible = visibleValue
  hs.timer.waitUntil(
    function() return view ~= nil end,
    function()
      hs.timer.waitWhile(
        function() return view ~= nil and view:loading() end,
        function()
          if view ~= nil then
            view:show()
